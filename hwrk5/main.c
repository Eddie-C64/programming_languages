...
{
    //Check if we have room in our movie list to add the movie.
    while(30 >= count)
        {
            //Add the title
            list[count].name = movie_name;
            //If the rating is higher then 10 and below 0 throw and error.
            if((rating =< max) && (0 < ratting){
                list[count].rating = rating;
                count = count++;
            }else{
                printf("ERROR, Rating is invalid!\n");
            }
        }
}
